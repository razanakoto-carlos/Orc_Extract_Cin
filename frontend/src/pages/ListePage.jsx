...
export default function ListePage() {
  ...
  const handleSearchByCIN = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/documents/db/search/${searchTerm}`);
  ...
  const handlePhotoSearch = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/face/search?threshold=0.4&top_k=10`,
        formData,
        {
          headers: {
      ...
      const matchingIds = response.data.matches.map(match => match.document_id);
      ...
      setError(err.response?.data?.detail || "Erreur lors de la recherche par photo");
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F1FAFF] to-[#CBE4FF] p-6">
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">📋 Liste des Documents</h1>
      ...
      {photoSearchMode && (
        <div className="mb-8 bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-4">🔍 Recherche par Photo</h2>
          <div className="flex flex-col md:flex-row gap-6 items-center">
            ...
            <div className="flex-1 space-y-4">
              <div>
                <h3 className="font-medium text-gray-700 mb-2">Comment ça marche ?</h3>
                <ul className="text-sm text-gray-600 space-y-1 list-disc pl-5">
                  <li>Téléchargez une photo de visage</li>
                  <li>Le système compare avec toutes les photos dans la base de données</li>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-medium text-blue-800">
                        {photoSearchResults.length} correspondance{photoSearchResults.length !== 1 ? 's' : ''} trouvée{photoSearchResults.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                    <button
      ...
      {filteredDocuments.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredDocuments.map((doc) => {
            ...
            const photoMatch = photoSearchResults.find(result => result.document_id === doc.id);
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-100 mt-1">
                    {selectedDocument.type_document || "CIN"} • N° {selectedDocument.numero_cin || "N/A"}
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedDocument.nom} {selectedDocument.prenoms}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Numéro CIN</label>
                    <p className="font-mono font-medium">{selectedDocument.numero_cin || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Date de naissance</label>
                    <p className="font-medium">{formatDate(selectedDocument.date_naissance)}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Lieu de naissance</label>
                    <p className="font-medium">{selectedDocument.lieu_naissance || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Sexe</label>
                    <p className="font-medium">{selectedDocument.sexe === "M" ? "Masculin" : selectedDocument.sexe === "F" ? "Féminin" : "Non spécifié"}</p>
                  </div>
                </div>

                {/* Document Information */}
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-gray-800 border-b pb-2">📄 Informations du Document</h3>
                  <div>
                    <label className="text-sm text-gray-500">Type de document</label>
                    <p className="font-medium">{selectedDocument.type_document || "Carte d'Identité Nationale"}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Date de délivrance</label>
                    <p className="font-medium">{formatDate(selectedDocument.date_delivrance)}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Date d'expiration</label>
                    <p className="font-medium">{formatDate(selectedDocument.date_expiration)}</p>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Statut</label>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      getDocumentStatus(selectedDocument.date_expiration).color === "green" ? "bg-green-100 text-green-800" :
                      getDocumentStatus(selectedDocument.date_expiration).color === "orange" ? "bg-orange-100 text-orange-800" :
                      "bg-red-100 text-red-800"
                    }`}>
                      {getDocumentStatus(selectedDocument.date_expiration).label}
                    </span>
                  </div>
                  <div>
                    <label className="text-sm text-gray-500">Adresse</label>
                    <p className="font-medium">{selectedDocument.adresse || "N/A"}</p>
                  </div>
                </div>

                {/* Photo Section if available - SHOW ACTUAL PHOTO */}
                {selectedDocument.has_face_photo && selectedDocument.photo_visage_path && (
                  <div className="md:col-span-2 mt-4">
                    <h3 className="text-lg font-semibold text-gray-800 border-b pb-2 mb-4">📸 Photo du Document</h3>
                    <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl p-4 border border-gray-200">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-sm text-gray-600">Photo extraite du document</span>
                      ...
                      <div className="text-center">
                        <div className="inline-block bg-white p-2 rounded-lg shadow-sm">
                          <div className="w-48 h-48 bg-gray-100 rounded-lg overflow-hidden">
                            <img 
                              src={getPhotoUrl(selectedDocument.photo_visage_path)} 
                              alt="Face photo"
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                e.target.style.display = 'none';
                      ...
                      <p className="text-xs text-gray-500 text-center mt-3">
                        Photo stockée: {selectedDocument.photo_visage_path?.split('/').pop() || "N/A"}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="md:col-span-2 pt-4 border-t">
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">📊 Métadonnées</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">ID Document</div>
                      <div className="font-mono font-medium">{selectedDocument.id}</div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">Dossier</div>
                      <div className="font-medium truncate">{selectedDocument.folder_name}</div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">Enregistré le</div>
                      <div className="font-medium">{formatDate(selectedDocument.date_sauvegarde)}</div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">Photo</div>
                      <div className="font-medium">{selectedDocument.has_face_photo ? "✓ Disponible" : "✗ Non disponible"}</div>
                    </div>
                  </div>
                </div>
              </div>
